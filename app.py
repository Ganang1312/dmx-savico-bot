...
# Lấy thông tin nhạy cảm từ các biến môi trường bạn đã cài trên Render
CHANNEL_ACCESS_TOKEN = os.environ.get('CHANNEL_ACCESS_TOKEN')
CHANNEL_SECRET = os.environ.get('CHANNEL_SECRET')
GOOGLE_CREDS_JSON = os.environ.get('GOOGLE_CREDENTIALS_JSON')

# Kiểm tra xem các biến đã được thiết lập đầy đủ chưa
...
def parse_competition_data(header_row, data_row):
    ...
    for category_name, indices in category_indices.items():
        if len(indices) == 3: # Chỉ xử lý ngành hàng có đủ 3 cột (%HT, Realtime, Target)
            try:
...
def create_flex_message(store_data, competition_results):
    ...
    if competition_results:
        ...
        title_box = {"type": "box", "layout": "horizontal", "margin": "md", "contents": [
        ...
        for item in competition_results:
            component = {"type": "box", "layout": "horizontal", "margin": "md", "contents": [
    ...
    flex_json = {
        "type": "flex", "altText": f"Báo cáo Realtime cho siêu thị {ma_sieu_thi}",
        "contents": {
            "type": "bubble", "size": "giga",
            "header": {"type": "box", "layout": "vertical", "contents": [{"type": "text", "text": "BÁO CÁO REALTIME", "weight": "bold", "size": "xl", "color": "#FFFFFF", "align": "center"}], "backgroundColor": "#006c83", "paddingAll": "20px"},
